...
## 1. Notation and the two explicit conventions the table relies on
...
| 8 | **Calibration deviation (coverage)** | – | `δ_j = |Cov_j − 0.95|` (either CATE or ATE row) | `θ_8 = E[|Cov − 0.95|]`, target `≈ 0` with a declared **equivalence band** `[0, Δ_cal]` | `mean(δ_j)` | `s_δ/sqrt(J)` | `D_j = δ_{A,j} − δ_{B,j}` | equivalence (TOST) on the paired means, or describe | This is how an over-/under-covering method is judged; it is **not** "lower coverage is better" (IJDA #10). The band `Δ_cal` is fixed before analysis (e.g. `±0.01`); see `tisca_v2_spec.md` §1 mode M5. |

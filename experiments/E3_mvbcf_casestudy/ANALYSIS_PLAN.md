...
- **Pilot:** the first 50 replications of each cell use the **independent pilot seed block**
  (`seed_cell_master = 2`, emitted seeds `1_000_001…1_000_050`). Pilot rows are used **only**
  to estimate `σ̂_D`, `Σ̂_D` and the DGP's `sd(τ(X))`; they are **discarded from the final
  inference** (plan P1-T2 §5).
- **Confirmatory:** `J` replications at `seed_cell_master = 1`, seeds `0…J−1`. `J_final = max`
